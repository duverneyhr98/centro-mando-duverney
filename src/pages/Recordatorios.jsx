    ...
    <div>
      ...
      <div className="bg-white rounded-xl shadow p-4 mb-6 space-y-3">
        <div className="flex gap-2 flex-wrap">
          <input
            type="text"
            placeholder="¿Qué debes recordar? (ej: Pago tarjeta Bancolombia)"
            value={titulo}
              ...
              <div key={r.id} className="bg-white rounded-xl shadow p-4 flex items-center justify-between flex-wrap gap-2">
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-1 flex-wrap">
                    ...
                    {r.negocios && (
                      <span className="text-xs bg-purple-100 text-purple-700 px-2 py-1 rounded-full">{r.negocios.nombre}</span>
                    )}
                  </div>
                  <p className="font-medium text-gray-800">{r.titulo}</p>
                  <p className="text-sm text-gray-500">
                    Vence: {new Date(r.fecha_vencimiento).toLocaleDateString("es-CO")} · {formatoCOP(r.monto)} · {r.frecuencia}
                  </p>
                </div>
                <div className="flex gap-3">
                  <button onClick={() => marcarPagado(r)} className="text-sm text-green-600 hover:underline">
                    Marcar pagado
                  </button>
                  <button onClick={() => eliminarRecordatorio(r.id)} className="text-sm text-red-500 hover:underline">
                    Eliminar
                  </button>
                </div>
              </div>
